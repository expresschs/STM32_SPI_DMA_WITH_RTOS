...
static void w25qxx_delay(uint32_t ms)
{
    osDelay(ms);
}

static inline void w25qxx_spi_cs_low(void)
{
    spi_dev_cs(0, 0, 0);
}

static inline void w25qxx_spi_cs_high(void)
{
    spi_dev_cs(0, 0, 1);
}

static inline uint8_t w25qxx_spi_transfer(uint8_t *send, uint8_t *recv, uint32_t size)
{
    return spi_dev_transfer(0, send, recv, size);
}
